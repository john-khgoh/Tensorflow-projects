...
series_valid_bkp_out = np.array(series_valid_bkp)
#s_output = StandardScaler().fit(series_valid_bkp_out)
#s_output = MinMaxScaler().fit(series_valid_bkp_out)

#Create a separate scaler for each variable
s_output_d = UnStandardScaler()
s_output_t = UnStandardScaler()
_ = s_output_d.fit(series_valid_bkp_out[:,0].reshape(-1,1))
_ = s_output_t.fit(series_valid_bkp_out[:,1].reshape(-1,1))
rnn_forecast_d = s_output_d.inverse_transform(rnn_forecast[:,0].reshape(-1,1))
rnn_forecast_t = s_output_t.inverse_transform(rnn_forecast[:,1].reshape(-1,1))
rnn_forecast = np.stack((rnn_forecast_d,rnn_forecast_t),axis=1)

naive_forecast = np.array([mean[0]] * len(rnn_forecast_d)).flatten()

#mae_naive = tf.keras.metrics.mean_absolute_error(series_valid_bkp['Demand'], naive_forecast).numpy()
mape_rnn_d = tf.keras.metrics.mean_absolute_percentage_error(series_valid_bkp['Demand'], rnn_forecast_d.flatten()).numpy()
mape_rnn_t = tf.keras.metrics.mean_absolute_percentage_error(series_valid_bkp['Temperature'], rnn_forecast_t.flatten()).numpy()
#print(np.array(series_valid_bkp['Demand']),rnn_forecast)
#print(mae_naive," ",mae_rnn)
print(mape_rnn_d)
print(mape_rnn_t)

plot_series(time_valid, rnn_forecast_d.flatten(), label='rnn_forecast')
