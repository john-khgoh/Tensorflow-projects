...
#Normalization
m_train = MinMaxScaler()
series_train = m_train.fit_transform(series_train)
m_valid = MinMaxScaler()
series_valid = m_valid.fit_transform(series_valid)
...
m = MinMaxScaler()
m.fit(np.array(series_valid_bkp).reshape(-1,1))
series_valid = np.array(m.inverse_transform(series_valid)).flatten()
rnn_forecast = np.array(m.inverse_transform(rnn_forecast)).flatten()

naive_forecast = np.array([np.mean(series_valid_bkp)] * len(series_valid_bkp))
