...
rnn_forecast = rescale(rnn_forecast,list_of_means[0],list_of_sds[0])
rnn_forecast_df = pd.DataFrame(rnn_forecast,columns=['LAX_Delayed'])
...
mae_rnn = tf.keras.metrics.mean_absolute_error(series_valid_copy['LAX_Delayed'],rnn_forecast_df['LAX_Delayed']).numpy()
print(mae_naive,mae_rnn)

plt.figure(figsize=(10, 6))
plot_series(time_valid, series_valid_copy['LAX_Delayed'])
plot_series(time_valid, rnn_forecast_df['LAX_Delayed'])
plt.show()