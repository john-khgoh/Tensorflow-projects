...
x = tf.keras.layers.ZeroPadding2D(padding=(1,0))(tf.expand_dims(inputs,axis=0))
x = tf.keras.layers.Conv2D(conv_layer_filters,(3,3),padding='valid',activation='relu')(x)
x = tf.keras.layers.MaxPooling2D(2,2)(x)
x = tf.keras.layers.UpSampling2D(size=(2,1))(x)
x = tf.keras.layers.Dense(dense_layer[0],activation='relu')(tf.squeeze(x,axis=0))
...
rnn_forecast_df = pd.DataFrame(np.squeeze(np.array(rnn_forecast)[:,:,1])).transpose()
