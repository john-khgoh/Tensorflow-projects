#Univariate with tf.keras.utils.timeseries_dataset_from_array()
...
model = tf.keras.models.Sequential([ 
        tf.keras.layers.Conv1D(filters=conv_layer_filters,kernel_size=5,strides=1,activation='relu',input_shape=[None,1]),
...
#rnn_forecast = np.concatenate((rnn_forecast,dummy_np),axis=1)

#Reducing the dimension of the output using squeeze() and flipping the rows & columns to get a normal DF shape (transpose)
rnn_forecast = np.squeeze(minmaxscaler.inverse_transform(rnn_forecast))
...
series_valid = np.squeeze(minmaxscaler.inverse_transform(series_valid))
...
#The model is a phase shift version of original. Check on how to overcome this. 