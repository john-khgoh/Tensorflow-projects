...
#plot_series(time, series, label='whole')
#plt.show()
...
#rnn_forecast = rescale(rnn_forecast,mean,std)
...
plot_series(np.flip(time_valid), np.flip(rnn_forecast), label='rnn_forecast')
plot_series(np.flip(time_valid), np.flip(naive_forecast), label='naive_forecast')
plt.legend()
plt.show()
