...
#checkpoint_filepath = '/kaggle/working/model.ckpt'
wd = getcwd()
...
series_sds_list = list(series.std())
...
for i,j in enumerate(series):    
    series[j] = abs(series[j]-series_mean_list[i])/series_sds_list[i]
...
rnn_forecast_df = pd.DataFrame(rescale(rnn_forecast,series_mean_list[0],series_sds_list[0]),columns=['price actual'])
...
mae_rnn = tf.keras.metrics.mean_absolute_error(series_valid['price actual'],rnn_forecast_df['price actual']).numpy()
...
plot_series(time_valid, series_valid['price actual'])
plt.show()
#plt.savefig(output + '1.png')

plot_series(time_valid, rnn_forecast_df['price actual'])
